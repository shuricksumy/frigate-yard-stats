...
def test_claim_visit_thumb_crop_batch_object_types_include_filter(conn_ok):
    ...
    db._execute(
        "UPDATE yard_stats.visits SET thumb_crop_status = 'new' WHERE id = ANY(%s)",
        ([car_visit_id, person_visit_id],),
    )
    try:
        claimed_ids = {
            r["id"] for r in db.claim_visit_thumb_crop_batch(limit=10, object_types=["car"])
        }
...
def test_claim_visit_thumb_crop_batch_exclude_object_types_filter(conn_ok):
    car_id, car_det = _insert_raw_event(objects="car")
    person_id, person_det = _insert_raw_event(objects="person")
    car_visit_id = db.record_visit({
        "camera": "pytest-cam", "zone": "pytest-zone", "objects": "car",
    ...
    try:
        claimed_ids = {
            r["id"] for r in db.claim_visit_thumb_crop_batch(limit=10, exclude_object_types=["person"])
        }
        assert car_visit_id in claimed_ids
        assert person_visit_id not in claimed_ids
    finally:
