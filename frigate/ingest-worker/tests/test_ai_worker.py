...
def test_run_once_dedups_every_mapped_type_by_default(monkeypatch):
    # AI_ONLY_VISIT_REPRESENTATIVE's hardcoded fallback is True (a deliberate default change from
    # this project's prior always-analyze-everything behavior) -- with no per-type or defaults:
    # override anywhere in the profile, both mapped types land in one single claim_ai_batch call
    # with only_visit_representative=True, not two separate calls.
    monkeypatch.setattr(config, "AI_EVENTS_STAGE_ENABLED", True)
    monkeypatch.setattr(config, "AI_ONLY_VISIT_REPRESENTATIVE", True)
    calls = []

    def fake_claim(object_types, parallel_limit, stale_minutes, max_age_hours=None, only_visit_representative=False):
        calls.append((set(object_types), only_visit_representative))
        return []

    monkeypatch.setattr(db, "claim_ai_batch", fake_claim)
    ai_worker.run_once(PROFILE)
    assert calls == [({"car", "person"}, True)]


def test_run_once_splits_claim_by_per_type_only_visit_representative(monkeypatch):
    # car opts out of dedup (e.g. it's the type actually flooding and the deployment wants to
    # investigate every duplicate individually) while person keeps the True default -- this can't
    # be expressed as one claim_ai_batch call (only_visit_representative is one bool for the whole
    # call), so run_once must issue two separate calls, one per resolved value.
    monkeypatch.setattr(config, "AI_EVENTS_STAGE_ENABLED", True)
    monkeypatch.setattr(config, "AI_ONLY_VISIT_REPRESENTATIVE", True)
    profile = {
        "object_types": {
            "car": {**PROFILE["object_types"]["car"], "ai_only_visit_representative": False},
            "person": PROFILE["object_types"]["person"],
        },
    }
    calls = []
    ...
    ai_worker.run_once(profile)
    assert len(calls) == 2
    assert ({"person"}, True) in calls
    assert ({"car"}, False) in calls


def test_run_once_respects_global_default_off_for_only_visit_representative(monkeypatch):
    # A deployment that sets defaults: {ai_only_visit_representative: false} -- e.g. to restore the
    # exact prior behavior across the board -- gets one call with the flag off for every type,
    # not split at all (both types resolve to the same value).
    monkeypatch.setattr(config, "AI_EVENTS_STAGE_ENABLED", True)
    monkeypatch.setattr(config, "AI_ONLY_VISIT_REPRESENTATIVE", True)
    profile = {
