...
# Default true: uses Frigate's own already-rendered event snapshot (GET
# /api/events/{det_id}/snapshot.jpg) instead of seeking+cropping a frame from the record-stream
# clip ourselves -- Frigate picks this frame by its own best-detection-score judgment
# (content-dependent, not a fixed offset guess like CROP_FRAME_OFFSET_PCT above), which in
# practice beats our own fixed-offset seek often enough that this is the default rather than an
# opt-in. Known trade-off, accepted as the default anyway: it's from the lower-res detect stream
# (800x448 in testing, vs. this setup's 3840x2160 record stream) with a bounding-box/label/
# timestamp overlay burned in that this Frigate version's API gives no way to suppress (confirmed
# empirically -- bbox=0/timestamp=0/h=<n> query params on the snapshot endpoint have no effect at
# all, byte-identical response). Set to false to fall back to this project's original seek-based
# approach if that trade-off doesn't work for your footage. When true, CROP_DISABLED/
# CROP_FRAME_OFFSET_PCT/CROP_PADDING_PCT above stop applying to events entirely -- there's no
# frame-seeking or region-cropping happening on our side anymore, Frigate's own snapshot is used
# as-is; they still apply if this is set back to false. Events only -- a visit's own composite grid
# (VISIT_THUMB_CROP_ENABLED) is unaffected either way, since a single Frigate snapshot has no
# "4 frames of change" equivalent to offer it.
FRIGATE_SNAPSHOT_ENABLED = _env("FRIGATE_SNAPSHOT_ENABLED", "true").lower() == "true"
