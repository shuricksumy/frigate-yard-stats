...
def generate_report(
    ...
    include_preview: bool = Query(True, description="Only relevant with source=visits. True (default) embeds a visit's animated preview GIF inline when it's ready (today's behavior). False skips fetching/embedding the GIF entirely (falls back to the static composite grid/crop instead) for a lighter-weight report -- e.g. an n8n workflow that emails/messages the result and wants a smaller payload."),
):
