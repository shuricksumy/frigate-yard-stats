...
def main():
    ...
    # Only spins up a thread at all when video storage OR Telegram-video-without-storage is
    # wanted for at least one object type -- no polling overhead when nothing needs it, rather
    # than a thread that runs and no-ops forever. Checks
    # profile_config.any_video_events_worker_needed (storage OR Telegram video -- see that
    # module's "send-to-Telegram-without-storing" section) rather than the bare global flag -- a
    # profiles.yaml per-type override can start this thread even when the global default is off.
    if profile_config.any_video_events_worker_needed(profile):
        threading.Thread(target=video_worker.run_forever, args=(profile,), name="video_worker", daemon=True).start()
    # Independent switch, independent thread -- the alerts flow can be on/off regardless of the
    # events flow, so the two can be A/B'd separately. Same per-type-override-can-start-it check
    # as above.
    if profile_config.any_video_alerts_worker_needed(profile):
